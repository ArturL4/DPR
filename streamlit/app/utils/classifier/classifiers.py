...
from utils.visu_tools.plot import plot_results
...
class NearestMeanClassifier:
    ...
    def predict(self, x_test):
        ...
        for x in x_test:
            ...
            distances = np.sum(np.abs([x - self.means[0], x - self.means[1]]),axis=1)
...
class KNearestNeighborClassifier:
    ...
    def predict(self, x_test):
        ...
        for x in x_test:
            ...
            knn_labels = sorted_labels[:self.k].astype(int)
...
def eval_classifiers(x_train, y_train, x_test, y_test):
    ...
    # plot results
    return plot_results(x_train, y_train, x_test, y_test, confusion_matrices, classifiers)